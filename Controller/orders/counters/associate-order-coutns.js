...
export const getOrderCountsForAssociate = async(req,res)=>{
        try {
...
const getPendingOrders = async (req) => {
    try {
        const sql = `SELECT COUNT(*) AS count FROM orders WHERE sellerId = ? AND status = ?`;
...
const getRecievedOrders = async (req) => {
    try {
        const sql = `SELECT COUNT(*) AS count FROM orders WHERE sellerId = ?`;
        const result = await new Promise((resolve, reject) => {
            conn1.query(sql, [req.body.id], (err, result) => {
                if (err) {
                    reject(err);
                } else {
