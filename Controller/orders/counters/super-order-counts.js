...
export const getOrderCountsForSuper = async (req, res) => {
...
const getPendingOrders = async (req) => {
    try {
        const sql = `SELECT COUNT(*) AS count FROM orders WHERE superId = ? AND status = ?`;
...
const getRecievedOrders = async (req) => {
    try {
        const sql = `SELECT COUNT(*) AS count FROM orders WHERE superId = ?`;
        const result = await new Promise((resolve, reject) => {
